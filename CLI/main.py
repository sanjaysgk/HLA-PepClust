...
def _print_credits(credits=False):
    ...
    if credits:
        ...
        text.append("GibbsCluster - 2.0 (Simultaneous alignment and clustering of peptide data)\n", style="bold link https://services.healthtech.dtu.dk/services/GibbsCluster-2.0/")
        text.append("Seq2Logo - 2.0 (Visualization of amino acid binding motifs)\n", style="bold link https://services.healthtech.dtu.dk/services/Seq2Logo-2.0/")
        text.append("MHC Motif Atlas (MHC motif PSM matrics are genrated using mhcmotifatlas please cite [link])\n\n", style="bold link http://mhcmotifatlas.org/home")
    else:
        text.append("HLA-PEPCLUST", style="bold link https://www.monash.edu/research/compomics/")
        text.append(f" (v{__version__})\n", style="bold")
    if credits:
        text.append("HLA motif finder pipeline for identifying peptide motif immunopeptididomic data.\n", style="italic")
...
def main():
    ...
    parser.add_argument("gibbs_folder", type=str,
                        help="Path to the test folder containing matrices.")
    parser.add_argument("reference_folder", type=str,
                        help="Path to the human reference folder containing matrices.")

    parser.add_argument("-hla", "--hla_types", nargs="*", default=None,
                        help="List of HLA types to search (defaults to all).")
    parser.add_argument("-p", "--processes", type=int,
                        default=4, help="Number of parallel processes to use.")
    parser.add_argument("-n", "--n_clusters", type=str,
                        default="all", help="Number of clusters to search for.")
    parser.add_argument("-k", "--best_KL", action="store_true",
